 ...
 */
    ...
    public function handleCallback(Request $request)
    {
        ...
        if ($oldStatus === 'success' && $newStatus !== 'success') {
            Log::info('Preventing status downgrade from success', [
                'order_id' => $orderId,
                'old_status' => $oldStatus,
                'attempted_status' => $newStatus
            ]);
            return response()->json(['message' => 'Status downgrade from success prevented'], 200);
        }
    ...
    public function updateSubscription(Subscription $subscription)
    {
        ...
        $duration = $subscription->package->duration_days ?? 30;
    ...
    public function verifyPaymentStatus($orderId)
    {
        try {
            Config::$serverKey = config('midtrans.server_key');
            Config::$isProduction = config('midtrans.is_production');
            /** @var object{transaction_status: string, fraud_status?: string, payment_type?: string} $status */
