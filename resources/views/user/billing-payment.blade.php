...
@section('meta_description',
    'Aktifkan langganan SINOMU sesuai kebutuhan unit Anda. Tersedia pilihan paket Basic dan
    Premium dengan fitur lengkap untuk otomasi notifikasi dan pengelolaan data.')

@section('og_description',
    'Berlangganan SINOMU dengan mudah melalui pembayaran online menggunakan Midtrans. Dapatkan
...
@section('content')
    <div
        ...
        <div class="card w-full max-w-md wadb1 shadow-2xl borderprimary">
            <div class="card-body relative overflow-hidden">
                ...
                <div class="text-center mb-8 relative z-10">
                    <div class="avatar mb-4">
                        <div class="w-20 h-20 rounded-full wadbo3 flex content-center justify-center">
                            <svg class="w-10 h-10 wad4 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="flex items-center justify-center gap-2 mb-2">
                        <h1 class="text-2xl font-bold wad4">
                            {{ $package->name }}
                        </h1>
                    </div>
                    @if (isset($package->description))
                        <p class="text-white text-sm mb-4">{{ $package->description }}</p>
                    @endif
                </div>
                <div class="text-center mb-8">
                    <div class="wadb4 rounded-2xl p-6 border border-yellow-100">
                        <div class="text-sm wad1 mb-1">Total Pembayaran</div>
                        <div class="text-4xl font-bold wad1">
                            Rp{{ number_format($package->price, 0, ',', '.') }}
                        </div>
                        @if (isset($package->duration_days))
                            <div class="text-sm mt-2">
                                <div class="badge wadb1 wad4 badge-sm">{{ $package->duration_days }} hari</div>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                @endif
                <button id="pay-button" class="btn btnprimarys btn-lg w-full group relative overflow-hidden">
                ...
                <div class="flex items-center justify-center gap-4 mt-6 pt-4 border-t border-base-300">
                    <div class="flex items-center gap-1 text-xs text-base-content/60">
                        ...
                        <span class="wad4">Keamanan SSL</span>
                    </div>
                    <div class="flex items-center gap-1 text-xs text-base-content/60">
                        ...
                        <span class="wad4">Midtrans</span>
                    </div>
                </div>
                <div class="text-center mt-4">
                    <a href="{{ route('user.billing') }}" class="btn btnsecondarys btn-sm">
        ...
        <div class="modal-box text-center">
            ...
            <div class="modal-action justify-center">
                <button class="btn btn-success"
                    onclick="window.location.href='{{ route('user.billing') }}'">Lanjutkan</button>
            </div>
        </div>
    </dialog>
    <dialog id="error-modal" class="modal">
        <div class="modal-box text-center">
            ...
            <div class="modal-action justify-center">
                <button class="btn btn-error" onclick="window.location.href='{{ route('user.billing') }}'">Coba
                    Lagi</button>
            </div>
        </div>
    </dialog>
    <dialog id="pending-modal" class="modal">
        <div class="modal-box text-center">
            ...
            <div class="modal-action justify-center">
                <button class="btn btn-warning" onclick="window.location.href='{{ route('user.billing') }}'">Periksa
                    Status</button>
            </div>
        </div>
    </dialog>
